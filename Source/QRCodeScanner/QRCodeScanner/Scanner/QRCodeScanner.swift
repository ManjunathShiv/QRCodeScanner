...
public final class QRCodeScanner: NSObject {
    /// Internal Properties.
    let sessionQueue = DispatchQueue(label: "com.manjunath.session.queue")
    let metadataObjectsQueue = DispatchQueue(label: "com.manjunath.qr", attributes: [], target: nil)
                ...
                if let _readableCodeObject = current as? AVMetadataMachineReadableCodeObject {
                    if _readableCodeObject.stringValue != nil {
                        if weakSelf.metadataObjectTypes.contains(_readableCodeObject.type) {
                            ...
                            let scannedResult = QRCodeScannerResult(value: sVal, metadataType:_readableCodeObject.type.rawValue)
...
extension QRCodeScanner {
    ...
    public class func supportsMetadataObjectTypes(_ metadataTypes: [AVMetadataObject.ObjectType]? = nil) throws -> Bool {
        guard let captureDevice = AVCaptureDevice.default(for: .video) else {
            throw NSError(domain: "com.manjunath.error", code: -1001, userInfo: nil)
        }
